...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        ...
        b = (double *)malloc(rows * cols * sizeof(double));
    ...
    if (rank == 0) {
        a_local = (double *)malloc(local_rows * cols * sizeof(double));
        b_local = (double *)malloc(local_rows * cols * sizeof(double));
        c_local = (double *)malloc(local_rows * cols * sizeof(double));
        int offset = 0;
        ...
        for (int p = 0; p < size; p++) {
            ...
            if (p == 0) {
                ...
                MPI_Send(&rows_p, 1, MPI_INT, p, 0, MPI_COMM_WORLD);
                MPI_Send(a + offset, rows_p * cols, MPI_DOUBLE, p, 0, MPI_COMM_WORLD);
                MPI_Send(b + offset, rows_p * cols, MPI_DOUBLE, p, 0, MPI_COMM_WORLD);
            }
        ...
        MPI_Recv(a_local, local_rows * cols, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(b_local, local_rows * cols, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    ...
    if (rank == 0) {
        ...
        for (int p = 0; p < size; p++) {
            ...
            if (p == 0) {
                ...
                MPI_Recv(c + offset, rows_p * cols, MPI_DOUBLE, p, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }

            offset += rows_p * cols;
        }
    } else {
        MPI_Send(c_local, local_rows * cols, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
    }
